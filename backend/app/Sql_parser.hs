...
import Data.Aeson (eitherDecode, encode, decode, ToJSON, toJSON, Value, Value(String), Value(Number))
...
data Table = Table [Row] deriving (Show)
...
instance ToJSON Table where
    toJSON (Table rows) = toJSON rows

instance ToJSON Row where
    toJSON (Row xs) = toJSON $ HM.fromList $ map (\(c, v) -> (T.pack c, v)) xs
